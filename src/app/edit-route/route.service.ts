...
export class RouteService {
  ...
  private loadRoute(routeID: string): Promise<Route> {
    return new Promise<Route>((resolve, reject) => {
      ...
      req.open(
        'GET',
        'https://kamelong.com/nodeJS/api/route?routeID=' + routeID,
        false
      );
      req.send(null);
    });
  ...
  public setRouteByName(routeName: string) {
    ...
    req.open(
      'GET',
      'https://kamelong.com/nodeJS/api/route?routeName=' + routeName,
      false
    );
    req.send(null);
  }
  ...
  public selectStation(stationID: string) {
    if (this.stationInsertMode) {
      ...
      this.route
        .getValue()
        .routeStations.splice(this.posInsertStation, 0, station);
