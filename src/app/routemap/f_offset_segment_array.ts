  ...
  try {
    ...
    while (0 < segmentList.length) {
      ...
      for (let i2 = 0; i2 < segmentList.length; i2++) {
        ...
        if (
          0 < i2 &&
          i2 < segmentList.length - 1 && //最初と最後は残すので除く
          segmentList[i2].st > segmentList[i2].et && //順序が逆
          segmentList[i2]['sn'] === false &&
          segmentList[i2]['en'] === false //残す指定なし
        ) {
          //逆の順序の場合
...
function f_offset_3(segmentList: Segment[]) {
  ...
  const c_vnx =
    segmentList[segmentList.length - 1].ex -
    segmentList[segmentList.length - 1].sx;
  const c_vny =
    segmentList[segmentList.length - 1].ey -
    segmentList[segmentList.length - 1].sy;
  const c_vnn = (c_vnx * c_vnx + c_vny * c_vny) ** 0.5;
  segmentList[segmentList.length - 1].exy = [
    new Point(
...
function f_offset_2(segment1: Segment, segment2: Segment) {
  ...
  for (let i1 = 0; i1 < 1; i1++) {
    c_xy.push(
      new Point(
        c_s1z * c_z.xy[i1].x[0] + c_s2z * c_z.xy[i1].x[1] + c_z.xy[i1].x[2],
        c_s1z * c_z.xy[i1].y[0] + c_s2z * c_z.xy[i1].y[1] + c_z.xy[i1].y[2]
      )
    );
  }
  // segment1.exy = c_xy;
...
function f_offset(segment1: Segment, segment2: Segment): Offset {
  ...
  offset.xy.push(
    new Point3(
      [v1y / v1r / v1r / a, v2y / v2r / v2r / a, (p12x + p21x) * 0.5],
      [-v1x / v1r / v1r / a, -v2x / v2r / v2r / a, (p12y + p21y) * 0.5]
    )
  );
  return offset;
}
