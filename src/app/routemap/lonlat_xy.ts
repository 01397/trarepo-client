...
export function f_lon_to_x(a_lon: number, a_zoom_level: number): number {
  return (a_lon / 180) * 6378137 * Math.PI;
  // return (a_lon / 180 + 1) * 0.5 * (2 ** a_zoom_level);
}
export function f_lat_to_y(a_lat: number, a_zoom_level: number): number {
...
export function f_x_to_lon(a_x: number, a_zoom_level: number): number {
  return (a_x / Math.PI / 6378137) * 180;
  // return (a_x / (2 ** a_zoom_level) / 0.5- 1) * 180;
}
export function f_y_to_lat(a_y: number, a_zoom_level: number): number {
